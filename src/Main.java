...
public class Main {
	...
	public static void main(String[] args) {
		...
		Empresa empresa3 = new Empresa(3, "SafeWay Restaurante", "53239160000150", 0.20, 0.0);
					...
					case 1: {
						...
						do {
							...
							produtos.stream().forEach(x -> {
								if (x.getEmpresa().getId().equals(escolhaEmpresa) && x.getQuantidade() > 0) {
									System.out.println(x.getId() + " - " + x.getNome());
								}
							});
						...
						Cliente clienteLogado = clientes.stream()
								...
								.collect(Collectors.toList()).get(0);
						Venda venda = Venda.criarVenda(carrinho, empresaEscolhida, clienteLogado, vendas);
